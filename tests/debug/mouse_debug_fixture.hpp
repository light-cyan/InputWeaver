...
inline debug::CaptureStartedPayload MouseDebugCapture()
{
    ...
    capture.sources = {{"path", EventTransition::Move, ExpressionType::Number},
    ...
    capture.mouse.sources.resize(3);
    auto& path = capture.mouse.sources[0];
    ...
    auto& pulse = capture.mouse.sources[1];
    ...
    pulse.moving = true;
    auto& wheel = capture.mouse.sources[2];
