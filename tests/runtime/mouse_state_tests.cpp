...
struct Periods {
    ...
    static RuntimeEvaluationResult Evaluate(void* context, EventSourceId source) noexcept
    {
...
double Field(const RuntimeMouseState& mouse, EventField field, std::uint32_t source = kInvalidProgramIndex,
    bool completed = false, std::span<const MouseCycle> selection = {})
{
    const auto result = mouse.Read({EventSourceId{source}, field, completed}, selection);
...
{
    const std::array configs{MouseSourceConfig{}, MouseSourceConfig{}};
    RuntimeMouseState mouse(configs, {80'000'000});
    Periods periods{Number(8), Number(24)};
    mouse.Initialize({100, 200}, 0);
    Check(Field(mouse, EventField::Period, 0) == 0 && Field(mouse, EventField::StartX, 0) == 100,
    ...
    Check(Field(mouse, EventField::StartX, 1, true) == 100
        && Field(mouse, EventField::X, 1, true) == 124
        && Field(mouse, EventField::StartX, 1) == 124
        && Field(mouse, EventField::X, 1) == 127
        && Field(mouse, EventField::Distance, 1) == 3,
    ...
    Check(Field(mouse, EventField::Valid, 0, true, empty) == 0, "empty selection remains invalid");
    mouse.Refresh({127, 200}, 200'000'000);
    Check(Field(mouse, EventField::Distance, 1) == 3 && Field(mouse, EventField::Moving, 1) == 0,
        "ordinary idle preserves distance progress");
    Input(mouse, periods, Move(-24, 0, 103, 200), 201);
    Check(Field(mouse, EventField::Dx, 0, true, first) == 8
        && Field(mouse, EventField::Dx, 0, true) == -8,
        "reserved completion selection remains stable after later inputs");
    mouse.Restart(EventSourceId{0});
    Check(Field(mouse, EventField::Valid, 0, true) == 0 && Field(mouse, EventField::Valid, 1, true) == 1
        && Field(mouse, EventField::Dx, 0, true, first) == 8,
...
void TestDynamicPeriodsAndTurns()
{
    ...
    Check(Field(mouse, EventField::Period, 0, true) == 80 && Field(mouse, EventField::Period, 0) == 20
        && Field(mouse, EventField::Dx, 0, true) == 35 && Field(mouse, EventField::Dy, 0, true) == 45,
    ...
    Check(Near(Field(mouse, EventField::Distance, 0), 5) && Field(mouse, EventField::Remaining, 0) == 15,
        "path distance uses Euclidean segments");
}

void TestTimePhase()
{
    const std::array configs{MouseSourceConfig{EventTransition::Move, ExpressionType::Duration}};
    RuntimeMouseState mouse(configs, {80'000'000});
    ...
    Check(mouse.Occurrences().size() == 1 && Field(mouse, EventField::Progress, 0) == 3
        && Near(Field(mouse, EventField::Distance, 0, true), 101)
        && Near(Field(mouse, EventField::StartX, 0), 101),
    ...
    Check(Field(mouse, EventField::Progress, 0) == 0 && Field(mouse, EventField::Moving, 0) == 0
        && Field(mouse, EventField::Valid, 0, true) == 1,
        "idle timeout clears unfinished time while retaining the latest completion");
    Input(mouse, periods, Move(1, 0, 205, 0), 283);
    Check(mouse.Occurrences().empty() && Field(mouse, EventField::Progress, 0) == 0,
    ...
    Check(mouse.Occurrences().size() == 3 && Field(mouse, EventField::Progress, 0) == 5,
        "one input confirms multiple time periods and retains the remainder");
}

void TestWheelAndLiveObservation()
{
    const std::array configs{MouseSourceConfig{EventTransition::Wheel}, MouseSourceConfig{EventTransition::HorizontalWheel}};
    RuntimeMouseState mouse(configs, {80'000'000});
    Periods periods{Number(1), Number(1)};
    mouse.Initialize({20, 30}, 0);
    mouse.Refresh({20, 30}, 10'000'000);
    Check(Field(mouse, EventField::Moving) == 0 && Field(mouse, EventField::IdleTime) == 10,
    ...
    Check(Near(Field(mouse, EventField::Progress, 0), 0.15)
        && Field(mouse, EventField::Dx) == 0 && Field(mouse, EventField::WheelX) == 0
        && Field(mouse, EventField::WheelY) == -0.1,
    ...
    Check(mouse.Occurrences().size() == 2 && Near(Field(mouse, EventField::Progress, 0), 0.25),
    ...
    Check(Field(mouse, EventField::WheelX, 1, true) == -1 && Field(mouse, EventField::Progress, 1) == -0.5
        && Field(mouse, EventField::WheelY, 1, true) == 0, "horizontal wheel preserves its axis and sign");
    mouse.Refresh({200, 300}, 91'000'000);
    mouse.ResetSources();
    Check(Field(mouse, EventField::Moving) == 0 && Field(mouse, EventField::IdleTime) == 80
        && Field(mouse, EventField::WheelX) == -1.5 && Field(mouse, EventField::X) == 200,
