...
看到 `Compiled program is active` 后切回刚才的记事本。切换期间的输入会直接放行；规则只在所选记事本处于前台时响应。此次运行会覆盖并重新写入 `example\acceptance.jsonl`。
...
在任意窗口按物理组合键 Ctrl+Shift+F12。控制台应打印 `Compiled-program session stopped`，并且 `injection_failures=0`、`hook_log_drops=0`、`injection_log_drops=0`、`runtime_log_drops=0`、`jsonl_truncated=false`。

不要再次运行 `example\run.bat`，否则会覆盖本次日志。完成后告诉我“验收完成”，并附上肉眼观察到的不一致；我会读取 `example\acceptance.jsonl`，按照物理输入、运行时状态转换、输出控制编号、注入成功数和取消原因逐项核验。
