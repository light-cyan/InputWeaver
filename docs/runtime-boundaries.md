...
可执行文件目标使用一个共享的 Windows 进程定位器。没有目录分隔符或驱动器标记的选择器按可执行文件基本名匹配；带路径结构的选择器必须是绝对路径，并在规范化后按完整映像路径匹配。两类比较都使用 Windows 序号、不区分大小写的比较方式。只有一个匹配实例时直接选中；多个匹配实例中恰好有一个位于前台时选中该实例；否则 Target 等待匹配变为可确定，并列出候选 PID 与路径。

`--exclude-process <pid-or-exe-name-or-absolute-path>` 使用同一定位器增加一个排除实例。纯十进制选择器直接转为非零 32 位 PID；文件名或绝对路径必须在启动时立即定位为一个可确定的运行实例，否则执行器报错退出。定位结果只保存 PID，不按名称或路径跟踪后续进程；被排除实例重启后产生的新 PID 不在当前排除范围内。

排除 PID 位于前台时，运行时路由拒绝输入分派和新的输出，低级钩子最终放行物理输入；Windows 注入边界会再次拒绝新的按下或重复输出。安全清理产生的释放不受排除限制。这个边界独立于目标类型，因此 Global 运行也可以排除一个前台进程。TUI 为它启动的每个执行器自动传入交互终端宿主 PID。
...
捕获开始消息携带 `PAUSE` 和全部用户 `state`、`number`、`duration` 的完整值快照，值数量上限为 12289；之后只发送实际变化。生产队列溢出时，执行器停止当前捕获并报告 `DebugStreamOverflow`，DebugClient 把派生状态标记为不可信并请求新的捕获代次；规则执行本身继续运行。
