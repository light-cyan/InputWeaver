...
编译产物保存由程序结构精确推导出的需求。激活错误输出中的 `code`、`required` 和 `available` 分别表示失败类别、程序需求和执行器容量；容量检查失败不会安装输入钩子，也不会开始规则分派。
...
持续就绪任务达到 16 个调度量子后，工作线程最多退避 1 毫秒。新输入、目标资格变化、取消、关闭和物理强制停止会提前唤醒工作线程，控制台的 `scheduler_backoffs` 记录实际发生的退避次数。

普通输出速率只统计按下和重复转换。达到速率边界时，产生超额输出的任务或映射被取消并清理所有权；输出释放不占用普通输出额度，因此速率耗尽不会阻止必要的键抬起。

## 输入线程等待边界

低级输入钩子不会等待后台任务释放共享状态。如果规则求值所需的 PAUSE 状态或用户变量正在被后台任务独占更新，本次物理事件直接放行；执行器不会为了等待状态锁而阻塞系统输入线程。

任务唤醒是非阻塞通知。输入分派、目标资格变化、取消和关闭只发布唤醒状态，不等待任务线程完成；任务线程会消费已经累计的工作和状态变化。

## 取消代次与输出所有权

运行程序使用单调递增的取消代次标记任务、映射和待发布输出。PAUSE 状态变化、目标丢失、重新加载、强制停止、关闭和致命运行故障会推进代次并使旧工作失效。
...
`--log` 启动时覆盖指定 JSONL 文件。日志生产者不会等待磁盘写入；记录环满时丢弃新记录并增加相应丢弃计数。文件达到 8 MiB 后停止追加完整记录并设置 `jsonl_truncated=true`，规则执行和物理强制停止仍继续工作。
