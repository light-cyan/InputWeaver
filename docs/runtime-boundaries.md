...
Target 退出时，执行器取消目标相关任务和映射并释放输出，随后在同一个已激活运行时中等待并绑定新的匹配进程；用户变量、数组和 PAUSE 状态继续保留。
...
带 Debug 会话令牌启动时，`InputWeaver.exe` 在等待 Target 之前建立 Debug pipe，并在整个执行器生命周期内保持同一连接；Target 退出和重新绑定不会重建 Debug pipe 或 Debug 捕获状态。
