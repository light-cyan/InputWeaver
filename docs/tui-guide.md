...
STATE 先显示全部用户 `state`、`number` 和 `duration` 当前值以及全部数组，再显示当前按下的控制；`off`、`0` 和空数组也会显示。数组始终在名称后显示逻辑长度，分别显示为 `[values[0]=[]]`、`[values[3]=[2, 4, 8]]`，长数组显示为 `[values[1000]=[0, 1, 2, 3, ..., 996, 997, 998, 999]]`。超过 STATE 区域宽度的项目会换行，并随 STATE viewport 一起滚动。控制项同时标出其当前来源，例如 `[LCtrl PHY]` 或 `[F22 INIT]`。
