...
Debug 页包含带边框的 EVENTS、PRESSED、ACTION EXECUTIONS 和 HEALTH 区域。`[Tab]` 在前三个可滚动区域之间切换焦点；使用 `[Up]`、`[Down]`、`[PageUp]`、`[PageDown]`、`[Home]` 和 `[End]` 滚动当前焦点区域。

Debug 页使用 `[C]` 开始或停止捕获，使用 `[X]` 停止 Debug 执行器，使用 `[Left]` 或 `[Q]` 返回 Programs 页。这些按键作用于整个 Debug 页，不属于某个信息区域。

EVENTS 使用对齐的时间、控制、来源和处置结果列，并在其后显示 `REPEAT` 与 `NO-DOWN` 标记。

PRESSED 使用多列显示当前处于按下状态的控制，并支持与其他可滚动区域相同的滚动按键。

ACTION EXECUTIONS 显示匹配时间、放在方括号中的触发事件、动作指令、当前指令颜色、最近三条已执行指令的颜色，以及不带下划线的最终执行状态颜色。

HEALTH 始终在独立边框内显示连接状态、捕获状态、信任状态、捕获代次、Debug 故障、运行时问题数量和最近问题。没有 Debug 故障或运行时问题时使用健康颜色；出现问题时改用配置的橙红色故障颜色。
