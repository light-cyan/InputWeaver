...
VARIABLES shows user scalars and arrays. INPUT STATE shows held keys and mouse buttons together with live Mouse observation. METERS summarizes named current/latest cycles using grouped coordinates and progress/period pairs. EVENTS displays keyboard and mouse-button transitions and meter ticks, grouping consecutive ticks from the same meter and repeated down reports from the same control. Exact input and cycle histories support task correlation independently of display grouping. ACTION EXECUTIONS shows each execution's trigger, condition, and action. Capture start, stop, and recovery preserve meter phase, completion identities, and physical movement history.
