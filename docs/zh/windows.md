...
| 控制表示 | 输入观察 | 物理状态查询 | 输出 |
| --- | --- | --- | --- |
| [命名键盘键和多媒体键](rules.md#section-key-name-reference) | 按物理报告分派 `down`、`again`、`up` | 支持 | 按下、松开和重复按下 |
...
部分原始扫描码可接收事件，但启动时无法转换为虚拟键来查询物理状态。这类来源先观察到一次物理释放，再建立输入基线；使用它的 `held` 或 `idle` 条件时，启动能力检查会报告失败。[启动时的按键状态](running.md#section-keys-already-held-at-startup)
...
命令解释器的管道或重定向由明确启动的解释器处理。进程创建成功后，`exec` 继续执行后续动作；本次启动授权和模拟运行行为见[启动外部程序](actions.md#section-launch-an-external-program)。
