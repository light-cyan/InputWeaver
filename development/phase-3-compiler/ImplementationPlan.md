...
- Implement the complete Weave v1 token set with longest matching for arrows, assignment, and comparison operators.
...
- Represent settings, typed declarations, complete mappings, event rules, conditions, action calls, action gaps, `if`, `repeat`, `while`, and expressions.
...
- Apply the language defaults and closed bounds for `TAP_DURATION` and `ACTION_GAP`, require declaration initializers to be same-type literals, and reject writes to read-only built-in durations.
...
- Parser tests cover every production, precedence level, nesting form, empty action flow, action adjacency, and recovery boundary.
- Binder tests cover every symbol rule, built-in, action signature, operator signature, target form, value type, and writable-value rule.
- Lowering tests cover every expression opcode, action opcode, rule kind, arrow combination, mapping form, range, requirement, control-use bit, debug span table, empty-action invalid ID, task-local repeat-frame index, and yielded backward edge.
...
- Golden tests compile the three Phase 2 fixture sources and match the frozen canonical dumps exactly.
...
The compiler may implement work that does not depend on an active issue, but it may not declare completion while an issue in `development/OpenDesignIssues.md` can change Weave syntax, binding, lowering, control identity, or the frozen contract. The shared control catalog is a direct binding dependency. A pause-control decision becomes a compiler dependency if it introduces syntax or rule classification.
...
- The three canonical source files match the frozen Phase 2 fixture dumps exactly.
